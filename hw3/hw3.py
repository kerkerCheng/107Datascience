...
def grid_search(train_X, y):
    ...
    grid_result.to_csv(timestamp + '_ada_grid.csv', sep=',', encoding='utf-8')
...
def main():
    ...
    # clf = VotingClassifier(estimators=voting_models, n_jobs=-1)
    # clf.fit(train_X, y)
    # ans = clf.predict(test_X)
    #
    # with open(os.path.abspath('sub_' + timestamp + '.csv'), 'w+') as f:
    #     f.write('ID,ans\n')
